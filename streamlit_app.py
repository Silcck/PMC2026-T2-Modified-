...
st.sidebar.header("Indirect Costs")
fixed_overhead = st.sidebar.number_input("Daily Fixed Overhead Cost (€/day)", min_value=0.0, value=0.0, step=10.0)
variable_overhead = st.sidebar.number_input("Daily Variable Overhead Cost (€/day)", min_value=0.0, value=0.0, step=10.0)
...
if uploaded_file:
    ...
    project = ProjectNetwork(standardized_csv, fixed_overhead=fixed_overhead, variable_overhead=variable_overhead)
    ...
    with col1:
        st.metric("Baseline Total Time", f"{baseline_time:.2f} hours ({baseline_time/8:.2f} Work days)")
    with col2:
    ...
    if st.button("🚀 Optimize Project"):
        with st.spinner("Running optimization algorithms..."):
            ...
            with res_col1:
                st.metric("NEW Total Time", f"{new_time:.2f} hours ({new_time/8:.2f} Work days)", delta=f"{new_time - baseline_time:.2f} hours")
            with res_col2:
                st.metric("NEW Total Cost", f"€{new_cost:,.2f}", delta=f"€{new_cost - baseline_cost:,.2f}", delta_color="inverse")

            # 1. Cost vs. Time Trade-off Chart
            if project.optimization_history:
                st.subheader("📈 Cost vs. Time Trade-off")
                history_df = pd.DataFrame(project.optimization_history)
                
                fig_tradeoff = px.line(
                    history_df, 
                    x='Time', 
                    y=['Cost', 'Direct_Cost', 'Indirect_Cost'],
                    labels={'value': 'Cost (€)', 'Time': 'Project Duration'},
                    title="Cost Dynamics during Optimization",
                    markers=True,
                ...
                fig_tradeoff.update_layout(
                    ...
                    xaxis_autorange="reversed" if "Crash" in opt_mode else None
                )
            ...
            # 2. Tabs for AON and Gantt
            st.divider()
            st.subheader("🗺️ Visual Project Outputs")
            tab_aon, tab_gantt = st.tabs(["AON Diagram", "Gantt Chart"])
            ...
            with tab_gantt:
                # Prepare Gantt Data
                gantt_df = project.results.copy()
                gantt_df = gantt_df.sort_values(by=['ES', 'Task_ID'])
                
                # Mock dates for Plotly timeline
                base_date = pd.Timestamp("2026-01-01")
                gantt_df['Start_Date'] = base_date + pd.to_timedelta(gantt_df['ES'], unit='D')
                gantt_df['Finish_Date'] = base_date + pd.to_timedelta(gantt_df['EF'], unit='D')
                ...
                fig_gantt = px.timeline(
                    ...
                    y="Task_ID", 
                    color="is_critical",
                    hover_data=['Task_Name', 'Duration', 'Slack'],
                    color_discrete_map={True: 'red', False: '#636EFA'},
                    title="Optimized Project Schedule"
                )
            ...
            with col_ex1:
                csv_data = project.results.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="Download Optimized Dataset (CSV)",
                    data=csv_data,
                    file_name="optimized_project_data.csv",
                    mime="text/csv",
            ...
            with col_ex2:
                if dot_opt:
                    # Graphviz export
                    dot_string = dot_opt.source
                    st.download_button(
                        label="Download AON Diagram (DOT Source)",
                        data=dot_string,
                        file_name="optimized_aon_diagram.dot",
                        mime="text/vnd.graphviz",
                        use_container_width=True
                    )
                    
                    try:
                        # Attempt PNG generation if possible, else skip gracefully
                        png_data = dot_opt.pipe(format='png')
                        st.download_button(
                            label="Download AON Diagram (PNG)",
                            data=png_data,
                            file_name="optimized_aon_diagram.png",
                            mime="image/png",
                            use_container_width=True
                        )
                    except Exception as e:
                        st.caption("PNG export requires Graphviz 'dot' binary installed on the server. DOT source is available above.")
                else:
                    st.info("No diagram available for export.")
